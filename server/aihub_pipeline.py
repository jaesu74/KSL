...
def check_dependencies():
    """필요한 패키지 설치 확인"""
    dependencies = ['tensorflow', 'mediapipe', 'flask', 'flask_cors', 'opencv-python', 'numpy', 'tqdm', 'requests']
    missing = []
...
<head>
    ...
    <style>
        ...
        .source-info {{ margin-top: 20px; padding: 10px; background-color: #e8f5e9; border-radius: 4px; }}
    </style>
    ...
    <div class="container">
        ...
        <div class="card">
            ...
            <div class="source-info">
                <strong>데이터 출처:</strong> AI Hub에서 수집된 한국어 수화 데이터 (일부는 시뮬레이션 데이터일 수 있음)
            </div>
        </div>
...
def main():
    """메인 함수"""
    parser = argparse.ArgumentParser(description='AI Hub 수화 데이터 수집 및 모델 학습 파이프라인')
    parser.add_argument('--collect', action='store_true', help='AI Hub에서 데이터 수집 수행')
    ...
    parser.add_argument('--dataset', type=str, default='data/aihub_ksl_dataset.json',
                      help='데이터셋 경로 (기본값: data/aihub_ksl_dataset.json)')
    parser.add_argument('--aihub-key', type=str, help='AI Hub API 키')
    parser.add_argument('--aihub-dataset-id', type=str, help='AI Hub 데이터셋 ID')
    ...
    if args.aihub_key:
        os.environ['AIHUB_API_KEY'] = args.aihub_key
    ...
    try:
        ...
        if args.collect or run_all:
            collect_cmd = [
                sys.executable, 'aihub_collector.py',
                '--output', args.dataset
            ]
            
            if args.simulate:
                collect_cmd.append('--simulate')
            
            if args.aihub_key:
                collect_cmd.extend(['--api-key', args.aihub_key])
            
            if args.aihub_dataset_id:
                collect_cmd.extend(['--dataset-id', args.aihub_dataset_id])
            
            if args.words:
                collect_cmd.append('--words')
                collect_cmd.extend(args.words)
            
            if args.samples:
                collect_cmd.extend(['--samples', str(args.samples)])
            
            print_header("AI Hub에서 한국어 수화 데이터 수집")
